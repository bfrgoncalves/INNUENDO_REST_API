...
class Job_queue(Resource):
	...
	def get(self):
		...
		request = requests.get(JOBS_ROOT, params={'job_id':args.job_id, 'username':str(current_user.username)})
		...
		if results != '':
			...
			if results['store_in_db'] == True:
				added, job_id = add_data_to_db(args.job_id, results['results'], current_user.id, args.procedure_name, args.sample_name, args.pipeline_id, args.process_position, args.project_id)
