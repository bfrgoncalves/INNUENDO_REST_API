...
process_post_parser.add_argument('messages', dest='messages', type=str,
                                 required=True, help="Process messages")
...
class ProcessListResource(Resource):
    ...
    def get(self, user_id, study_id, pipeline_id, process_id):
        ...
        processes = db.session.query(Process)\
            .filter(Process.pipeline_id == pipeline_id).all()
    ...
    def post(self, user_id, study_id, pipeline_id, process_id):
        ...
        protocol = Process(messages=jsonify(args.messages),
