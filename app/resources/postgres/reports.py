...
class ReportFilterResource(Resource):
	...
	def get(self):
		...
		reports = db.session.query(Report).filter(Report.project_id.in_(args.project_id.split(",")), cast(Report.timestamp, DATE) >= cast(args.minTimeFilter), cast(Report.timestamp, DATE) <= cast(args.maxTimeFilter)).all()
