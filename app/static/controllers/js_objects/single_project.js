function Single_Project(CURRENT_PROJECT_ID, CURRENT_PROJECT, $http, $rootScope){
	...
	function get_and_apply_pipeline(total_pipelines, pipeline_id, strain_id, project_id, added_pipeline_id, callback){
		...
		ngs_onto_requests.ngs_onto_request_applied_pipelines(pipeline_id, project_id, strain_id, function(response, strain_id){
        	console.log(response);
    ...
    var returned_functions = {
	    ...
	    get_workflows: function(classifier, callback){
			...
			pg_requests.get_workflows(classifier, function(response){
				//console.log(response);
				if(response.status == 200){
				...
				if (response.status == 200){
					for (i in response.data){
						if(response.data[i].parent_pipeline_id != null){
			                ...
			                ngs_onto_requests.ngs_onto_request_applied_pipelines(pipid, CURRENT_PROJECT_ID, sid, function(response, strain_id){
			                	//console.log(response);
					...
					if (response.status == 200){
						...
						for(pipeline in response.data){
							...
							if(response.data[pipeline].project_id == null || response.data[pipeline].parent_pipeline_id != null){
								...
								pipelines_ids.push([pipeline_id, projectid]);
								//total_pips[strainid]
								//console.log(pipeline_id, strainid);
								ngs_onto_requests.ngs_onto_request_applied_pipelines(pipeline_id, projectid, strain_id, function(response, strain_id){
									//console.log(response, strain_id);
		        			...
		        			while(dict_strain_names[strain_names[strain_name]][1].length != 0){
		        				...
		        				ngs_onto_requests.ngs_onto_request_get_workflow(pipelinesByName[workflowName], strain_name, function(response, strain_name){
			        				...
			        				if (dict_strain_names[strain_names[strain_name]][0] == dict_strain_names[strain_names[strain_name]][2]){
					        			...
					        			pg_requests.run_job(strains_dict[strain_names[strain_name]], indexes, strainID_pipeline[strains_dict[strain_names[strain_name]]], dict_strain_names[strain_names[strain_name]][6], strain_name, function(response, strain_name){
					...
					ngs_onto_requests.ngs_onto_request_get_jobid_from_process(strain_processes[s_p][1], [strain_processes[s_p][2]], strain_processes[s_p][0], strains[i].strainID, countStrain, function(response, pr_ids, strain_id, count_process, pip_id){
						...
						for(l in response.data){
							if(response.data[l].length != 0){
								...
								tasks_to_buttons[t_id] = strain_id.replace(/ /g, "_") + '_' + String(count_process[strain_id]) + '_' + CURRENT_PROJECT_ID;
								buttons_to_tasks[strain_id.replace(/ /g, "_") + '_' + String(count_process[strain_id]) + '_' + CURRENT_PROJECT_ID] = t_id;
								buttons_to_strain_names[strain_id.replace(/ /g, "_") + '_' + String(count_process[strain_id]) + '_' + CURRENT_PROJECT_ID] = strain_id;
