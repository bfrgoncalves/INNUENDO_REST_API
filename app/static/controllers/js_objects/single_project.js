		...
		apply_workflow: function(mode, type_proc, callback){
		    ...
		    for(i in strain_data){
		        ...
		        if(mode == 'new'){
	        		...
	        		'<button class="btn btn-sm btn-default dropdown-toggle workflows_child" shown_child="false" data-toggle="dropdown" strainID="'+strain_data[counter]['strainID']+'" name="'+proc_value+'" id="'+strain_data[counter]['strainID'].replace(/ /g, '_')+"_"+String(pip_start_id + 1)+ '_' + CURRENT_PROJECT_ID+'">'+ proc_value + '<i class="fa fa-arrow-down"></i></button>'+
