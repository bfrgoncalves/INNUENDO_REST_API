...
from config import wg_index_correspondece, core_index_correspondece, core_headers_correspondece, wg_headers_correspondece, allele_classes_to_ignore, BASE_ROOT
...
def send_to_phyloviz(job_ids, dataset_name, dataset_description, additional_data, database_to_include, max_closest, user_id, species_id, missing_data, missing_char):
	...
	if missing_data == "True":
		...
		command = 'python ./app/resources/phyloviz/remoteUpload.py -cd '+BASE_ROOT+' -root '+BASE_ROOT+' -sdt profile -sd ' + file_path_profile + ' -d "'+dataset_name+'" -dn "'+dataset_description+'" -m '+ file_path_metadata +' -pid ' + parent_id;
